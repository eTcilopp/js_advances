...
class Product {
    ...
    renderProduct() {
        return `<div id="${this.id}" class="product-item" style="visibility:visible;">
...
// Событие клика на кнопку поиска

document.getElementsByClassName('searchButton')[0].addEventListener('click', event =>searchField())
let field = document.getElementsByClassName('searchTerm')[0]
...
function searchField(){
    let searchText = field.value;
    let re = new RegExp(searchText, 'i');
    for (let id in catalogDict){
        if (catalogDict[id].title.match(re)){
